...
function getFilesizeInBytes() {
    ...
    return fileSizeInMegabytes;
            ...
            case '!shi3a':
                case '!10freakyshi3a':
                    client.sendFile(from, './media/shi3a.mp3', 'shi3a.mp3')
break
        case '!stickergif':
        case '#stickergif':
        case '!sgif':
            ...
            if (isMedia &&(mimetype === 'video/mp4' && message.duration < 10 || mimetype === 'image/gif' && message.duration < 10)) {
                ...
                await exec(`gify ${filename} ./media/output.gif --fps=30 --scale=240:240`, async function (error, stdout, stderr) {
                    ...
                    if (getFilesizeInBytes() > 4.3){
                        ...
                        client.reply(from, "Your gif/video it's too large, try to compress/trim it and send it again!", id)
                    } else {
    ...
    
{
     ...
     break
            ...
            client.sendLinkWithAutoPreview(from, '*Currently unavailable, Working on it, Comming soon.GO FUCKING WATCH ON HTTP://PORNHUB.COM*')
                    break
    ...
    const {
       ...
       if (nsfw_ == true) {
             if ((!isGroupMsg) && (isnsfw)) {
                       await client.sendFileFromUrl(from, `${url80}`, 'Reddit.jpg', `${title80}` + '\n\nPostlink:' + `${postLink80}`)
           ...
           console.log(err)
           await client.reply(from, 'There is no such subreddit!', id) 
       }
       break
       case '!pfp':
        case '!profilepic':
...
const he = require('he');
 const apiKey = AIzaSyAkxvlJs7qOZdUAjRdcl6Q1r3JxWw7b42k
    const videoName = args.join(' ');
    if (!videoName) return sendText('Hey, specify a video name and Ill search')
    const searchOptions = { maxResults: 1, key: apiKey, type: 'video' };
    if (!isnsfw) searchOptions['safeSearch'] = 'strict';
    let result = await search(videoName, searchOptions)
      .catch(err => {
        message.client.logger.error(err);
        return client.sendText('Error, Please try again later')
      });
    result = result.results[0];
    if (!result) 
      return sendText('Unable to find, try something else..')
    const decodedTitle = he.decode(result.title);
 
    client.sendText(result.description);
    break
case '!reddit':
    case '!sr':
    arg = body.trim().split(' ')
